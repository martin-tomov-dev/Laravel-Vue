...
class InvitationIntoTeamFolder extends Notification implements ShouldQueue
{
    ...
    {
        $appTitle = get_settings('app_title') ?? 'GoLink.co - Cloud Storage';
        ...
        if ($user) {
            return (new MailMessage)
                ...
                ->view('mails.notifications.invation-team-folder', [
                    'url' => url('/team-folder-invitation', ['id' => $this->invitation->id])
                ]);
                /*->greeting(__t('hello'))
        ...
        return (new MailMessage)
            ->subject(__t('team_invitation_notify_title', ['app' => $appTitle]))
            ->view('mails.notifications.invation-team-folder-noAccount', [
                'url' => url('/team-folder-invitation', ['id' => $this->invitation->id])
            ]);
            /*->greeting(__t('hello'))
