<!DOCTYPE html>
<html 
	class="{{ $config->app->installation === 'installation-needed' ? 'dark:bg-dark-background bg-light-background' : '' }}" 
...
<head>
    ...
    <title>
        {{ $config->app->title ?? 'GoLink.co - Cloud Storage' }} # {{ $config->app->description ?? __t('app_description') }}
    </title>
    ...
    <meta property="og:title" content="{{ $config->app->name ?? 'GoLink.co' }} | {{ $config->app->description ?? __t('app_description') }}">
    ...
    <meta name="apple-mobile-web-app-title" content="{{ $config->app->name ?? 'GoLink.co - Cloud Storage' }}">
    ...
    <script id="Cookiebot" src="https://consent.cookiebot.com/uc.js" data-cbid="9013e60a-397d-4cea-a596-5a8aaef56190" data-blockingmode="auto" type="text/javascript">
    </script>

	<script data-host="https://teklananalytics.com" data-dnt="false" src="https://www.teklananalytics.com/js/script.js" id="ZwSg9rf6GA" async defer></script>
...
<body style="min-height: 100%">
    ...
    <script>
		let config = {
            ...
            language: localStorage.getItem('language') ? localStorage.getItem('language') : '{{ isset($config->language) && $config->language ? $config->language : 'en' }}',
