<template>
    <div class="landing-page">
        ...
        <AuthContentWrapper v-if="isVisible" ref="auth">
            <!--Log In by Email-->
            <AuthContent name="log-in" :visible="true">
                ...
                <ValidationObserver @submit.prevent="logIn" ref="log_in" v-slot="{ invalid }" tag="form"
                    ...
                    <ValidationProvider class="w-full text-left" tag="div" mode="passive" name="E-Mail" rules="required"
                        v-slot="{ errors }">
                        <input
                            ...
                            :class="{ '!border-rose-600': errors[0] }" v-model="loginEmail"
                            :placeholder="$t('page_login.placeholder_email')" type="email" />
                        <span class="text-left text-xs text-red-600" v-if="errors[0]">{{ errors[0] }}</span>
                    </ValidationProvider>

                    <AuthButton class="w-full justify-center md:w-min" icon="chevron-right" :text="$t('next_step')"
            ...
            >-->
                <Headline v-if="checkedAccount" :title="$t('page_sign_in.title', { name: '***** ' })"
                    :description="$t('page_sign_in.subtitle')">
                    <img v-if="checkedAccount.avatar" class="user-avatar mx-auto mb-6 w-28 rounded-xl shadow-xl"
                        :src="checkedAccount.avatar.md" :alt="checkedAccount.name" />
                </Headline>

                <ValidationObserver @submit.prevent="singIn" ref="sign_in" v-slot="{ invalid }" tag="form"
                    class="mb-12 items-start space-y-4 md:flex md:space-x-4 md:space-y-0">
                    <ValidationProvider tag="div" mode="passive" class="w-full text-left" name="User Password"
                    ...
                    <AuthButton class="w-full justify-center md:w-min" icon="chevron-right" :text="$t('log_in')"
                        :loading="isLoading" :disabled="isLoading" />
                </ValidationObserver>
            ...
            <AuthContent name="not-verified" :visible="false">
                <Headline v-if="checkedAccount" :title="$t('page_sign_in_2fa_title', { name: checkedAccount.name })"
            ...
            <AuthContent name="two-factor-authentication" :visible="false">
                ...
                <ValidationObserver ref="two_factor_authentication" v-slot="{ invalid }" tag="form" class="mb-12">
                    <ValidationProvider tag="div" mode="passive" class="mx-auto" name="Two Factor Authentication"
                        rules="required" v-slot="{ errors }">
                        <input v-model="twoFactorCode" ref="twoFactorCodeInput"
                            :placeholder="$t('page_sign_in.placeholder_2fa')" @input="twoFactorChallenge(false)"
                            ...
                            :class="{ '!border-rose-600': errors[0] }" />
                        <span class="mt-2 block text-center text-xs text-red-600" v-if="errors[0]">{{ errors[0]
                        }}</span>
                    </ValidationProvider>
                </ValidationObserver>
            ...
            <AuthContent name="two-factor-recovery" :visible="false">
                ...
                <ValidationObserver ref="two_factor_recovery" v-slot="{ invalid }" tag="form" class="mb-12">
                    <ValidationProvider tag="div" mode="passive" class="mx-auto" name="Two Factor Recovery"
                        rules="required" v-slot="{ errors }">
                        <input v-model="twoFactorRecoveryCode"
                            :placeholder="$t('page_sign_in.placeholder_2fa_recovery')" @input="twoFactorChallenge(true)"
