...
const actions = {
    downloadZip: ({ commit, getters }, item = undefined) => {
        let files = []
...
// GoLink CHANGE HERE
        commit('PROCESSING_POPUP', {
			...
			message: i18n.t('toaster.creating_zip'),
		})
            ...
            axios
                ...
                .then(async (response) => {
                    ...
                    if (response.data.data.id) {
                        ...
                        //dispatch('getFolder', {page: 1, id: this.$route.params.id})
...
// GoLink END
    ...
    emptyTrash: ({ commit, getters }) => {
        ...
        commit('PROCESSING_POPUP', {
			title: i18n.t('toaster.please_wait'),
			message: i18n.t('toaster.permanently_removing_files'),
		})
        // Clear file browser
        commit('START_LOADING_VIEW')
...
// GoLink END
    ...
    pushFileToTheUploadQueue: ({commit, getters}, item) => {
        ...
        if (item.file.size === 0) {
            events.$emit('toaster', {
                type: 'danger',
                message: `File: - ${item.file.name} - with "0kb" can't be uploaded`,
            })
        }
